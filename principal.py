# principal.py - CON INTERFAZ ORIGINAL + FIX DEL ERROR
...
with st.sidebar:
    ...
    if SANDBOX_ZERO_ACTIVE:
        st.success("✅ SANDBOX 0 ACTIVO")
    else:
        st.error("❌ SANDBOX 0 INACTIVO")
    
    # Idioma
    idioma = st.selectbox("Language / Idioma", ["English", "Español"], key="lang_select")
...
caso = st.text_area(
    ...
    key="main_input_text"  # KEY ÚNICO AQUÍ
)

# Botón de análisis
if st.button(txt["btn"], type="primary", key="analyze_btn_primary"):
    if not caso:
        ...
        with st.spinner("🧠 Processing through Moralogy Framework..."):
            ...
            if DIVINE_LOCK_ACTIVE:
                ...
                if divine_result.get("decision") == "BLOCKED_BY_DIVINE_LOCK":
                    ...
                    if not st.checkbox("Continuar en modo limitado (sin preemptión)", key="continue_limited"):
                        st.stop()
            ...
            # ==================== 8. PROCESAR CON MORALOGY NORMAL ====================
            try:
...
with col1:
    if st.button("🔬 Advanced Analysis", key="adv_analysis_btn"):
        st.switch_page("pages/03_Analisis_Avanzado.py")

with col2:
    if st.button("🌟 Emergent Philosophy", key="emergent_phil_btn"):
        st.switch_page("pages/04_Emergent_Philosophy_Monitor.py")

with col3:
    if st.button("📊 Divine Lock Dashboard", key="divine_lock_btn"):
        st.switch_page("pages/06_Divine_Lock.py")

st.divider()
st.caption("Moralogy Engine v4.0 + Divine Lock + Sandbox 0 - Formal Ethics for the Age of AI")
